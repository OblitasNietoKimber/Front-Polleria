...
import {
  Flame, Search, ShoppingCart, Plus, Minus, Trash2, X, ArrowLeft,
...
export default function LenasYSaboresApp() {
  ...
  return (
    <div style={{ fontFamily: "'Work Sans', sans-serif", background: "var(--cream)", color: "var(--ink)", minHeight: "100%" }}>
      ...
      {selectedProduct && (
        <div
          style={{ position: "fixed", inset: 0, background: "rgba(27,21,18,0.5)", zIndex: 70, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}
        ...
        >
          ...
          >
            <button
              onClick={() => setSelectedProduct(null)}
              style={{ position: "absolute", top: 12, right: 12, background: "rgba(255,255,255,0.85)", border: "none", borderRadius: "50%", width: 30, height: 30, cursor: "pointer", zIndex: 2 }}
            >
              <X size={16} />
            </button>
            <div className="icon-tile" style={{ aspectRatio: "16/9" }}>
              <CategoryIcon id={selectedProduct.category} size={56} />
            </div>
            <div style={{ padding: 24 }}>
              {!selectedProduct.available && (
                <span className="font-mono" style={{ fontSize: "0.7rem", color: "#B23A2E", fontWeight: 700 }}>NO DISPONIBLE</span>
              )}
              <h3 className="font-display" style={{ fontSize: "1.4rem", fontWeight: 600, margin: "6px 0 10px" }}>{selectedProduct.name}</h3>
              <p style={{ color: "var(--smoke)", fontSize: "0.92rem", lineHeight: 1.6, marginBottom: 18 }}>{selectedProduct.desc}</p>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <span className="font-mono" style={{ fontSize: "1.3rem", fontWeight: 700, color: "var(--rust)" }}>{money(selectedProduct.price)}</span>
                <button
                  className="btn-ember"
                  disabled={!selectedProduct.available}
                  onClick={() => { addToCart(selectedProduct.id); setSelectedProduct(null); setCartOpen(true); }}
                >
                  Agregar al carrito
                </button>
              </div>
